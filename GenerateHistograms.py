...
def generate_histograms_bootstrapping(index, row, results_directory, total_hypotheses):
    ...
    rejected = (p_lb < 0.05/4/total_hypotheses) or (p_ub < 0.05/4/total_hypotheses)
...
def generate_longitudinal_plots(index, row, p_values, results_directory):
    ...
    rejected = min_p_value <= 0.05/4/len(p_values)
...
def main(args):
    ...
    for index, row in p_values.iterrows():
        generate_histograms_bootstrapping(index, row, args.image_export_dir, total_hypotheses=len(p_values))
