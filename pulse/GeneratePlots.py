...
def generate_longitudinal_plots_of_average_values(index, row, p_values, results_directory, total_hypotheses, caption=False, title=None, fig=None, axis=None):
    a = row['actions']
    age = row['age']
    gender = row['gender']
    x_t = row['x_t']
...
def generate_longitudinal_plots_combined(first_ind, second_ind, p_values, results_directory, total_hypotheses, col_name):
    fig, axis = plt.subplots(1, 2, figsize=(13,5), sharey=True)
    plt.style.use('fivethirtyeight')
    generate_longitudinal_plots_of_average_values(first_ind, p_values.iloc[first_ind], p_values, results_directory, total_hypotheses=total_hypotheses, caption=True, fig=fig, axis=axis[0], title=None)
    generate_longitudinal_plots_of_average_values(second_ind, p_values.iloc[second_ind], p_values, results_directory, total_hypotheses=total_hypotheses, caption=False, fig=fig, axis=axis[1], title=None)
    figtitle = f"{col_name}_hyp_{first_ind}v{second_ind}_longitudinal"
    plt.legend(fontsize=16, )
    plt.tight_layout()
    plt.savefig(f"{results_directory}/longitudinal/{figtitle}.pdf", format="pdf", bbox_inches='tight')  
    plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/data/ziz/not-backed-up/taufiq/HypothesisTesting/hyp_testing_new_pulse_data_2/images_check",
    )
